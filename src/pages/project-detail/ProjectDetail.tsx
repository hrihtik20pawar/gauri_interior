...
import { ArrowLeft, MapPin, Calendar, Layout, CheckCircle2 } from 'lucide-react';
      ...
      <div className="relative h-[60vh] min-h-[500px] w-full">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-[1500px] mx-auto w-full px-6 md:px-12 lg:px-24 pb-16">
            <Link to="/gallery" className="inline-flex items-center gap-2 text-white/80 hover:text-white transition-colors mb-6 font-medium text-sm bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
      ...
      <div className="max-w-[1500px] mx-auto px-6 md:px-12 lg:px-24 pt-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8 space-y-16">
            ...
            {/* Design Process */}
            <div>
              <h2 className="text-3xl font-serif text-brand-green mb-6">The Process</h2>
              <div className="space-y-6">
                {[
                  { title: "Initial Consultation & Concept", desc: "Understanding the client's vision and establishing the design language." },
                  { title: "Space Planning & 3D Visualization", desc: "Creating functional layouts and hyper-realistic renderings." },
                  { title: "Material Selection", desc: "Curating premium finishes, fabrics, and hardware." },
                  { title: "Execution & Handover", desc: "Seamless project management leading to a flawless final reveal." }
                ].map((step, i) => (
                  <div key={i} className="flex gap-4">
                    <div className="w-10 h-10 shrink-0 rounded-full bg-teal-50 flex items-center justify-center text-brand-teal font-bold border border-teal-100">
                      {i + 1}
                    </div>
                    <div>
                  ...
                  <ul className="mt-2 space-y-2">
                    {["Interior Design", "Custom Furniture", "Lighting Design", "Project Management"].map((s, i) => (
                      <li key={i} className="flex items-center gap-2 text-gray-700 font-medium text-sm">
                        <CheckCircle2 className="w-4 h-4 text-brand-teal" /> {s}
                      </li>
                    ))}
                  </ul>
                </li>
